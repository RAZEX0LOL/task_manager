...
const TaskCard = ({ task }) => {
  ...
  return (
    <>
      <div className='w-full h-fit bg-stone-900 dark:bg-[#1f1f1f] shadow-md p-4 rounded'>
        ...
        <>
          <Link to={`/task/${task._id}`}>
            <div className='flex items-center gap-2'>
              ...
              <h4 className='text- line-clamp-1 text-gray-200 dark:text-white'>
                {task?.title}
              </h4>
            </div>
          </Link>
        ...
        {task?.subTasks?.length > 0 ? (
          ...
          <div>
            <div className='py-4 border-t border-gray-200 dark:border-gray-700'>
              <span className='text-gray-500'>Нет подзадач</span>
            </div>
          </div>
        )}
