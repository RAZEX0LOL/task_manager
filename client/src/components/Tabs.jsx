        ...
        <Tab.List className='flex space-x-6 rounded-xl p-1'>
          {tabs.map((tab, index) => (
            <Tab
              ...
              className={({ selected }) =>
                classNames(
                  "w-fit flex items-center outline-none gap-2 px-3 py-2.5 text-base font-medium leading-5 bg-stone-900 dark:bg-[#1f1f1f]",

                  selected
                    ? "text-blue-700 dark:text-white border-b-2 border-blue-600"
                    : "text-gray-200 dark:text-gray-500 hover:text-blue-800"
                )
              }
            >
              {tab.icon}
