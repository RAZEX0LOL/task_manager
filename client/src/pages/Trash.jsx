...
import {AddUser, Button, ConfirmatioDialog, Loading, Title,} from "../components";
...
const Trash = () => {
  ...
  const deleteAllClick = () => {
    setType("deleteAll");
    setMsg("Вы уверены, что хотите безвозвратно удалить все задачи?");
    setOpenDialog(true);
  };

  const restoreAllClick = () => {
    setType("restoreAll");
    setMsg("Вы хотите восстановить все задачи из корзины?");
    setOpenDialog(true);
  };
  ...
  const restoreClick = (id) => {
    setSelected(id);
    setType("restore");
    setMsg("Вы хотите восстановить выбранную задачу?");
    setOpenDialog(true);
  };
  ...
  const TableHeader = () => (
      ...
      <tr className='text-black dark:text-white text-left'>
        ...
        <th className='py-2 line-clamp-1'>Дата изменения</th>
      </tr>
      </thead>
  );
      ...
      <tr className='border-b border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-400/10'>
        <td className='py-2'>
          <div className='flex items-center gap-2'>
            ...
            <p className='w-full line-clamp-2 text-base text-gray-200 dark:text-gray-400'>
              {item?.title}
            </p>
          </div>
        </td>
  ...
  return isLoading ? (
      ...
      <>
        <div className='w-full md:px-1 px-0 mb-6'>
          ...
          {data?.tasks?.length > 0 ? (
              <div className='bg-stone-900 dark:bg-[#1f1f1f] px-2 md:px-6 py-4 shadow-md rounded'>
              ...
              <div className='w-full flex justify-center py-10'>
                <p className='text-lg text-gray-200'>Нет удалённых задач</p>
              </div>
          )}
        </div>
